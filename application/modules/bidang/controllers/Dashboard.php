...
class Dashboard extends CI_Controller {
	...
	function index()
	{
		...
		$sm_n = $this->dashboard->suratmasuk_now($id_bidang)->jumlah;
		$sm_1 = $this->dashboard->suratmasuk_min1($id_bidang)->jumlah;
		$sk_n = $this->dashboard->suratkeluar_now($id_bidang)->jumlah;
		$sk_1 = $this->dashboard->suratkeluar_min1($id_bidang)->jumlah;
		$u_n = $this->dashboard->undangan_now($id_bidang)->jumlah;
		$u_1 = $this->dashboard->undangan_min1($id_bidang)->jumlah;
	...
	{
		$id_bidang = $this->session->userdata('id');
		$list = $this->undangan->get_data_jadwal($id_bidang);
		$data = array();
		$no = 1;
	...
	function view_surat($date = null)
	{
		$id_bidang = $this->session->userdata('id');
		// $tgl = date('Y-m-d', strtotime($date));
		$list = $this->surat_masuk->get_data($id_bidang);
		$data = array();
		$no = 1;
	...
	{
		$id_bidang = $this->session->userdata('id');
		$list = $this->undangan->get_data($id_bidang);
		$data = array();
		$no = 1;
		foreach ($list as $undangan) {
			$row = array();
